...
examples/              # Numbered demo scripts (0-20 match the online course; 21-23 IMU/trot; 24 tricks; 25 find)
  ...
  memory_extractor.py         # AgentExtractor: the memory pass as a Haiku structured-output query through the Agent SDK
...
Control channels. `control.ControlServer` listens on `/run/petronilo.sock` (mode 600, so `sudo`): one JSON request per connection, `say`, `ask`, `stop`, `remind`, `jobs`, `cancel`, `status`. `petronilo_ctl.py` is the client and the Makefile wraps it (`make ask MSG="..."`, `make say`, `make stop`, `make status`, `make jobs`); it is also the headless way to test him without the mic. `telegram_bridge.TelegramBridge` long-polls with the standard library only; set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_IDS` (list of ints) in `secret.py`. Unknown chats get their chat id back once so you can allowlist them; allowed chats get written answers (`run_task(..., speak=False)`), plus `/stop`, `/jobs`, `/status`. Barge-in by voice (`BARGE_IN`, off by default until tried on hardware): while he speaks inside a conversation, `BargeIn` (`voice_active_crawler.py`) listens on its own thread with the offline Vosk recogniser for the exact wake word as a whole word, none of the loose `WAKE_ALIASES`, since the USB mic hears mostly his own voice from the HAT speaker (separate ALSA devices, so both can be open). A hit runs `_on_barge_in`: `stop_speaking()`, `brain.interrupt()` and a flag that makes `_follow_up` listen for the new question at once (`BARGE_IN_LISTEN_SECONDS` when conversation mode is off). `_stop_talking()` joins the listener, re-setting the shared `stop_listening_event` until it has left `listen()`, and resets the recogniser before the follow-up opens the mic. It does not run during autonomous turns, where the library's wake-word thread already has the mic, and the legacy non-streaming `tts.say` cannot be cut mid-sentence.

Latency is the thing to watch. The session opens at the wake word (`brain.start`) so the CLI's start-up overlaps the listening, MCP tools load up front (`ENABLE_TOOL_SEARCH=false`, no ToolSearch turn), the prompt has him speak a sentence before any tool call, and a visual question (`_wants_image`) carries the camera frame in the user message (`brain.ask(..., image_path=)`, an image block ahead of the text) so it needs no `look` round trip. With those, on the Mac: first words about 2 s after the question, full answer with one tool call about 5 s.
