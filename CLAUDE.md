...
Extra `VoiceActiveCrawler` options used here: `stt=` (HybridSTT: offline Vosk for wake word / end of speech, `gpt-4o-transcribe` for the text), `follow_up_seconds` (keep listening after an answer without the wake word; those turns are sent with `FOLLOW_UP_TAG` and the model replies `IGNORAR` when they weren't meant for it, which stays silent and out of memory), `one_breath` (a question said with the wake word is answered directly; `HybridSTT` keeps the wake utterance's audio for cloud transcription), `end_phrases`, `stream_speech` (speak sentence by sentence while the LLM streams), `memory_file` / `memory_llm` (when a conversation ends, `memory.Memory.learn` sends the transcript to `memory_llm`, which returns add/update/delete edits to the stored facts plus a summary; the system prompt, pinned against history trimming, is rebuilt with them every turn), `battery_low_volts` / `battery_warning`. Wake word is "compa" with accent-insensitive near-miss aliases. Camera frames are sent only for visual questions.
