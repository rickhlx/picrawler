...
examples/              # Numbered demo scripts (0-20 match the online course; 21-23 IMU/trot; 24 tricks; 25 find)
  ...
  petronilo/setup_agent.sh    # Pi setup for the agent brain: unprivileged user, workspace, SDK, CLIs, skills
  petronilo/skills/           # Agent skills (weather, github, blogwatcher; adapted from OpenClaw), installed by setup_agent.sh
...
Two privilege levels. The robot tools (`move`, `find`, `look`, `sensors`) are an in-process MCP server that runs in the root voice service; `move` queues onto the action thread, so the body moves while he talks, capped by `max_actions`; its `steps` repeats walking, turning and trot up to the per-action limit in `VoiceActiveCrawler.REPEATABLE`, and each move ends on its own. The Claude Code CLI runs as `AGENT_USER` (`petronilo`) with cwd `AGENT_WORKSPACE`; Bash, skills (`~petronilo/.claude/skills/`) and external MCP servers (`examples/petronilo_mcp.json`, Claude Code's `mcpServers` format) run as that user. Every tool call passes a `PreToolUse` hook backed by `agent_policy.verdict`, with `permission_mode="dontAsk"` behind it: `can_use_tool` is not enough, because allow rules in settings files approve tools before it is consulted. The policy allows Bash only when every pipeline segment starts with a name in `AGENT_COMMANDS`, there are no redirects or substitutions, and no segment is a subcommand in `DENIED_SUBCOMMANDS` (`gh alias`/`extension`/`config` run other programs, `gh auth` prints the token), keeps Read/Glob/Grep inside the workspace and skills, keeps writes inside the workspace but out of its `.claude/` and `.mcp.json`, allows every tool of a configured MCP server, and denies anything else (subagents included). Never add a command that runs other commands (`sh`, `python3`, `xargs`, `env`, `sudo`) or reads or writes arbitrary files (`cat`, `grep`, `curl`); WebFetch covers HTTP. Setup: `sudo bash ~/picrawler/examples/petronilo/setup_agent.sh` on the Pi, then `ANTHROPIC_API_KEY` in `secret.py`.
...
- **Skill**: add the folder holding `SKILL.md` under `examples/petronilo/skills/`, `make sync`, and re-run `setup_agent.sh`, which copies them into `/home/petronilo/.claude/skills/` (a folder copied there by hand, and `chown -R petronilo:`, works too but isn't tracked). He can read a skill's files but runs only allowlisted commands, so skills that are instructions work; skills that ship scripts don't.
...
- **CLI tool**: install it from `setup_agent.sh` (next to `gh`, `jq` and `blogwatcher`), log it in as the agent's user (`sudo -u petronilo -H gh auth login`), make sure its name is in `AGENT_COMMANDS`, then `make deploy`. Never allowlist a command that runs others (`sh`, `python3`, `npx`, `xargs`, `env`, `sudo`) or reads or writes arbitrary files (`cat`, `grep`, `curl`): either one gets around the policy. If only some of its subcommands do that, allow it and list those in `agent_policy.DENIED_SUBCOMMANDS`.
