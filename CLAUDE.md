...
Failure and cost. `think()` never raises: if the agent fails before he has said anything, that turn is answered by the OpenAI LLM instead; if it fails mid-sentence he says `BRAIN_ERROR_PHRASE`. (The base `VoiceAssistant.run()` swallows exceptions and exits 0, which is why `petronilo.service` uses `Restart=always`.) Two spend caps: the SDK's `max_budget_usd` per conversation (`AGENT_BUDGET_USD`) and `daily_budget_usd` (`AGENT_DAILY_BUDGET_USD`), past which `AgentBrain.ask` raises `BudgetExceeded` and he says `BUDGET_PHRASE`. Each conversation logs its cost, turns and duration to the journal; `ResultMessage.total_cost_usd` is the *session's* running total (streaming input mode), so `_add_cost` adds only the increase per turn. Resuming does not restart that total, so `_connect` seeds the running total from the resumed session's spend, kept as `cost` in `agent_session.json` — without that, every resume bills the session's whole history again and compounds, which blows the daily cap on phantom spend. For the same reason the SDK's `max_budget_usd` is raised by the resumed session's spend, so it bounds one conversation rather than the session's history. `fallback_model` (`AGENT_FALLBACK_MODEL`, `claude-sonnet-5`) keeps him in the agent with his tools when `AGENT_MODEL` (`claude-fable-5-1`, effort `low`) is overloaded, before the OpenAI fallback. `MCP_TOOL_TIMEOUT` is raised so a long `find` is not abandoned by the CLI; the CLI's stderr goes to the journal as `(agente cli: ...)` and a `PreCompact` hook logs when a long session compacts.
